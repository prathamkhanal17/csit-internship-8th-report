...
\section{Introduction to the project/work}
...
\begin{itemize}
    \item To understand the organizational structure and operational protocols of a Network Operations Center (NOC).
    \item To differentiate between fiber optic cable types (Single-mode vs. Multi-mode) and identify standard connectors like SC, LC, and ST.
    \item To monitor network health by tracking latency, packet loss, and device status using the Zabbix monitoring tool.
    \item To manage customer relationships by logging, categorizing, and escalating trouble tickets within the CRM system.
    \item To configure Layer-2 switches, specifically focusing on VLAN creation and port assignment.
    \item To diagnose Fiber to the Home (FTTH) connectivity issues by analyzing optical power levels and signal attenuation.
    \item To apply basic network security measures, including the configuration of Access Control Lists (ACLs) and firewall rules.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Bibliography:} Lists additional resources studied during the internship.
