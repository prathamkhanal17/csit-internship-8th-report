...
\begin{itemize}
    \item \textbf{Network Monitoring:} Continuous monitoring of network health was performed using the Zabbix platform. This included observing uptime, latency, and packet loss for distribution switches in the Chitwan area.

    \item \textbf{Customer Support and CRM Management:} Responsibilities included handling the Customer Relationship Management (CRM) system to log trouble tickets. Technical issues were categorized into slow internet, loss of signal (LOS), or billing issues, effectively managing the incident lifecycle.
    ...
    \item \textbf{Fiber Diagnostics:} Fiber to the Home (FTTH) issues were diagnosed by analyzing optical power levels (Rx/Tx) and identifying customers experiencing high signal attenuation below the standard threshold of $-27$~dBm.
...
\begin{itemize}
    ...
    \item Studied company policies and security protocols related to server room access and physical security.
\end{itemize}
\end{minipage} \\ \hline
...
\begin{minipage}[t]{10cm}
\begin{itemize}
    \item Studied optical fiber hardware, differentiating between single-mode and multi-mode cables.
    \item Learned to identify connectors (SC, LC, and ST) and color codes (UPC and APC).
    \item Observed fiber splicing operations using a fusion splicer to ensure low insertion loss.
\end{itemize}
\end{minipage} \\ \hline
...
\begin{minipage}[t]{10cm}
\begin{itemize}
    \item Gained read-only access to the Zabbix monitoring dashboard.
...
\begin{itemize}
    ...
    \item Categorized issues and escalated complex problems to Level-2 engineers.
    \item Learned the importance of accurate documentation for faster issue resolution.
\end{itemize}
\end{minipage} \\ \hline
...
\begin{itemize}
    ...
    \item Created VLANs for data (VLAN 10) and voice (VLAN 20) traffic.
    \item Assigned switch ports using appropriate configuration commands like \texttt{switchport access vlan}.
\end{itemize}
\end{minipage} \\ \hline
...
\begin{itemize}
    ...
    \item Identified customers experiencing high attenuation levels below the $-27$~dBm threshold.
...
\begin{minipage}[t]{10cm}
\begin{itemize}
    \item Calculated subnetting schemes for corporate clients to ensure efficient IP usage.
    \item Configured static routing on test routers to direct traffic.
    \item Resolved IP conflicts caused by incorrect port connections or loops.
\end{itemize}
\end{minipage} \\ \hline
...
\begin{itemize}
    ...
    \item Analyzed system logs using \texttt{grep}, \texttt{tail}, and \texttt{chmod}.
    \item Verified DNS service status using \texttt{systemctl status named}.
\end{itemize}
\end{minipage} \\ \hline
...
\begin{minipage}[t]{10cm}
\begin{itemize}
    \item Studied firewall rules protecting the core network.
    \item Configured standard and extended ACLs to block specific IP ranges.
...
\begin{itemize}
    ...
    \item \textbf{Threshold Management:} Established that signal levels dropping below $-27$~dBm resulted in high attenuation and service instability, consistent with GPON standards.
    \item \textbf{Physical Remediation:} Guided field technicians to resolve issues identified by the data, such as cleaning dirty connectors or replacing patch cords that were bent beyond the allowable radius.
\end{itemize}
...
\textbf{Key Project Activities:}
\begin{itemize}
    \item \textbf{ACL Configuration:} Designed and deployed Standard and Extended Access Control Lists (ACLs) on routers to filter traffic and block specific malicious IP ranges.
    \item \textbf{Security Policies:} Implemented the security best practice of adding a default "Deny All" rule at the end of ACL sequences to ensure a fail-safe security posture.
...
\begin{itemize}
    \item Monitored distribution switches deployed across the Chitwan area to verify device availability and link status.
    ...
    \item Interpreted critical ``Red'' status triggers generated by Zabbix to promptly identify device outages, power failures, or link-down events at specific network nodes.
...
\begin{itemize}
    ...
    \item Configured Virtual LANs (VLANs) to logically segregate network traffic, assigning VLAN~10 for Data services and VLAN~20 for Voice services.
    ...
    \item Saved and backed up switch configurations using the \texttt{write memory} command to ensure persistence after device reboot.
\end{itemize}
...
\begin{itemize}
    ...
    \item Examined system and service logs located in \texttt{/var/log/messages} to identify warnings, errors, and unusual behavior using tools such as \texttt{grep}, \texttt{tail}, and \texttt{less}.
    \item Verified the operational status of essential network services, including DNS, using the command \texttt{systemctl status named}.
...
\begin{itemize}
    \item Identified and differentiated between Single-mode and Multi-mode fiber optic cables based on core size, color coding, and application scenarios.
    \item Recognized common fiber connector types such as SC, LC, and ST, and understood their usage in access and distribution networks.
    ...
    \item Observed fiber splicing operations using fusion splicers and verified splice quality to ensure low insertion loss and minimal signal reflection.
